<?php

namespace Database\Seeders;

use App\Models\SystemConfig;
use Illuminate\Database\Seeder;

class SystemConfigSeeder extends Seeder
{
    /**
     * Predefined SystemConfig Whitelist parameters for SIAKAD Al-Yasini.
     */
    public static array $whitelist = [
        'MAX_SKS_DEFAULT' => [
            'value' => '24',
            'type' => 'number',
            'category' => 'akademik',
            'description' => 'Batas maksimal SKS perkuliahan standar per semester',
        ],
        'MIN_PRESENSI_PERSEN' => [
            'value' => '75',
            'type' => 'number',
            'category' => 'akademik',
            'description' => 'Persentase minimal kehadiran kuliah untuk syarat mengikuti UAS (%)',
        ],
        'JUMLAH_PERTEMUAN_KULIAH' => [
            'value' => '16',
            'type' => 'number',
            'category' => 'akademik',
            'description' => 'Standar jumlah tatap muka pertemuan kuliah per semester',
        ],
        'STATUS_PERIODE_KRS' => [
            'value' => 'buka',
            'type' => 'select',
            'options' => ['buka', 'tutup'],
            'category' => 'krs',
            'description' => 'Status aktifasi gerbang pengisian KRS secara terpusat (buka / tutup)',
        ],
        'KRS_OPENING_DATE' => [
            'value' => '2026-08-01',
            'type' => 'date',
            'category' => 'krs',
            'description' => 'Tanggal pembukaan akses pengajuan KRS oleh mahasiswa',
        ],
        'KRS_CLOSING_DATE' => [
            'value' => '2026-08-31',
            'type' => 'date',
            'category' => 'krs',
            'description' => 'Tanggal penutupan akses pengajuan KRS oleh mahasiswa',
        ],
        'DENDA_UKT_PER_HARI' => [
            'value' => '5000.00',
            'type' => 'decimal',
            'category' => 'keuangan',
            'description' => 'Nominal denda keterlambatan pembayaran UKT per hari (Rp)',
        ],
        'CEKAL_KRS_OTOMATIS' => [
            'value' => 'ya',
            'type' => 'select',
            'options' => ['ya', 'tidak'],
            'category' => 'keuangan',
            'description' => 'Kunci akses KRS otomatis bagi mahasiswa yang memiliki tunggakan UKT',
        ],
        'MIN_BIMBINGAN_PROPOSAL' => [
            'value' => '8',
            'type' => 'number',
            'category' => 'skripsi_yudisium',
            'description' => 'Syarat minimal log konsultasi bimbingan proposal skripsi',
        ],
        'MIN_BIMBINGAN_SKRIPSI' => [
            'value' => '8',
            'type' => 'number',
            'category' => 'skripsi_yudisium',
            'description' => 'Syarat minimal log konsultasi bimbingan skripsi penuh',
        ],
        'MIN_IPK_YUDISIUM' => [
            'value' => '2.00',
            'type' => 'decimal',
            'category' => 'skripsi_yudisium',
            'description' => 'Syarat minimal IPK kumulatif kelulusan yudisium',
        ],
        'MIN_SKS_YUDISIUM' => [
            'value' => '138',
            'type' => 'number',
            'category' => 'skripsi_yudisium',
            'description' => 'Syarat minimal total SKS lulus untuk penetapan yudisium',
        ],
        'NAMA_INSTANSI_PENAUNG' => [
            'value' => 'Yayasan Pondok Pesantren Terpadu Al-Yasini',
            'type' => 'text',
            'category' => 'institusi',
            'description' => 'Nama instansi/yayasan penaung perguruan tinggi untuk kop surat',
        ],
        'KOTA_TERBIT_DOKUMEN' => [
            'value' => 'Pasuruan',
            'type' => 'text',
            'category' => 'institusi',
            'description' => 'Kota penerbitan dan penandatanganan dokumen akademik resmi',
        ],
    ];

    /**
     * Run the database seeds.
     */
    public function run(): void
    {
        foreach (static::$whitelist as $key => $meta) {
            SystemConfig::updateOrCreate(
                ['key' => $key],
                [
                    'value' => $meta['value'],
                    'description' => $meta['description'],
                ]
            );
        }
    }
}
